...
class HospitalManagementSystem:
    ...
    def show_modern_loading_screen(self):
        ...
        try:
            ...
            logging.error(f"Error loading loading screen assets: {e}")
...
# Placeholder methods for dashboards (unchanged)
    def show_admin_dashboard(self, user_id):
        ...
        # Show default view - FIXED: Actually call the method instead of just setting a command
    ...
    def show_admin_welcome(self):
        ...
        # Stats cards - arranged in a grid
        ...
        # Create stat cards in a 2-row grid
        stats = [
        ...
        for i, (title, value, card_type, command) in enumerate(stats):
            ...
            card.bind("<Leave>", lambda e, c=card: c.config(highlightbackground="", highlightthickness=0))
        ...
        for i, (text, color, command) in enumerate(action_buttons):
            btn = tk.Button(
                button_frame,
                text=text,
                font=("Arial", 11),
        ...
        # Recent activity section with cleaner design
        activity_frame = tk.Frame(main_container, bg="#ffffff", bd=1, relief=tk.SOLID, highlightbackground="#ecf0f1")
        ...
        # Create a styled treeview for activity log
        style = ttk.Style()
        ...
        columns = ("Date", "Time", "Activity", "User")
        ...
        # Configure columns
        for col in columns:
            self.activity_tree.heading(col, text=col, anchor=tk.W)
        ...
        self.activity_tree.tag_configure('oddrow', background="#f8f9fa")
        ...
        try:
            ...
            cursor.execute("""
                SELECT 
                    ...
                    action_description as activity, 
                    username 
                FROM system_logs 
            ...
            for i, activity in enumerate(activities):
                ...
                self.activity_tree.insert("", tk.END, values=activity, tags=(tag,))

        except Error as e:
            # Fallback with sample data
            sample_activities = [
                (datetime.now().strftime("%Y-%m-%d"), "10:30:00", "System login", "admin"),
                (datetime.now().strftime("%Y-%m-%d"), "09:15:00", "Added new doctor", "admin"),
                ((datetime.now() - timedelta(days=1)).strftime("%Y-%m-%d"), "14:20:00", "Updated patient record", "admin"),
                ((datetime.now() - timedelta(days=1)).strftime("%Y-%m-%d"), "11:45:00", "Generated report", "admin"),
                ((datetime.now() - timedelta(days=2)).strftime("%Y-%m-%d"), "16:30:00", "Added new staff member", "admin")
            ]
